...
| 平台 | 下载 | 说明 |
|---|---|---|
| 🍎 macOS（Apple Silicon） | `workbench-v1.6.0-mac-arm64.zip` | 解压后直接运行 `科研工作台.app`（⚠️ 见下方 macOS 说明） |
| 🪟 Windows x64 | `workbench-v1.6.0-win32-x64.zip` | 解压后运行 `科研工作台.exe`（**建议用 7-Zip 解压**，见下方 Windows 说明） |
...
- 产物：`dist/科研工作台-v1.6.0-mac-arm64.app`（arm64 / Apple Silicon）+ `.zip`（约 113MB）
- **Windows**：`npm run pack:win` → `dist/科研工作台-v1.6.0-win32-x64.zip`（约 125MB，内含主程序 `科研工作台.exe`；mac 上交叉打包走手动构造流程，含关键文件完整性校验 + 解压复验）
- **Release 上传**：GitHub 资产名仅支持 ASCII，发布时使用 `workbench-v1.6.0-mac-arm64.zip` / `workbench-v1.6.0-win32-x64.zip`（内容与本地中文名 zip 完全一致）
...
- 长按塞西悬浮球 520ms 开始录音，松开后先转写、净化并展示结果；只有用户点击「发送」后才会进入 Agent 历史或触发工具。
- ASR 默认使用本地 Whisper small Q5；开启回退后，本地失败才会把当次录音发送到配置的 OpenAI 兼容远程 ASR。
- TTS 默认使用 CosyVoice（也可选自定义远程 TTS）；远程失败后可回退到本地 MLX Qwen3-TTS。
- 本地 TTS 支持两种接口：「参考音频零样本」与「已微调说话人模型」。训练完成后，先将 PyTorch 权重转换为 MLX 格式，再在设置中选择模型目录、切换推理模式并填写说话人 ID。
- 模型、声音档案和临时录音位于 Electron `userData/voice-runtime/`，不放入安装包；临时录音会在成功、失败或取消后清理。
